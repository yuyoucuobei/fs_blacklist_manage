...
{    
    void *ppopdata = NULL;
    QUEUE_DATA_RECORD *pqueuedata = NULL;
    int threadcontinue = 1;
    char day[16] = {0};
    char cmd[256] = {0};
    int insertcount = 0;
    switch_interval_time_t timeout = 3000000;
    ...
    while (threadcontinue)
    {
        ...
        pqueuedata = (QUEUE_DATA_RECORD *)ppopdata;
        if(NULL == pqueuedata)
        {
            continue;
        }
        ...
        snprintf(cmd, sizeof(cmd), 
            "INSERT INTO bl_check_%s(userid, caller, callee, fee, hit) VALUE(%d, '%s', '%s', %f, %d)",
            day, pqueuedata->userid, pqueuedata->caller.c_str(), pqueuedata->callee.c_str(), 
            pqueuedata->fee, pqueuedata->hit);
...
switch_status_t blacklist_manage_record_init()
{
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, 
            "blacklist_manage_record_init success\n");
...
switch_status_t blacklist_manage_record_write(int userid, 
                                                           ...
                                                           int hit )
{
    QUEUE_DATA_RECORD *pdata = new QUEUE_DATA_RECORD();
    if(NULL == pdata)
    {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, 
                "QUEUE_DATA_RECORD new failed\n");
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, 
        "################ record work_queue_info[%d].pqueue, size=%d\n", queue_index, cursize);
